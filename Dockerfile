...
WORKDIR /usr/src/helixtelemetry/
...
COPY Pipfile* /usr/src/helixtelemetry/
...
ENV PROJECT_DIR=/usr/src/helixtelemetry
ENV FLASK_APP=helixtelemetry.api
...
COPY ./helixtelemetry ${PROJECT_DIR}

# Copy the Pipfile.lock from the previous stage in case it was locked
COPY --from=python_packages /usr/src/helixtelemetry/Pipfile.lock /usr/src/helixtelemetry/Pipfile.lock

# Copy Pipfile.lock to a temporary directory so it can be retrieved if needed
COPY --from=python_packages /usr/src/helixtelemetry/Pipfile.lock /tmp/Pipfile.lock
